This file is based on this repo. https://github.com/kuahyeow/git-workshop/tree/master 
...
First, we will focus of using git locally with a command line interface (CLI)
This will require the use of a terminal window (Linux, Mac) or of a Powershell window 
(Windows). Note for Windows users that "AnacondaPowershell" may work.  
...
First thing to do (if not done already) is to setup your identity. 
This identifies you to other people who download the project when you share it around.
First check your local setup: 

    $ git config --global --list
If configured, you will see something like: 

    user.email=youremail@email.com
    user.name=your name
If you have a github/gitlab/... account, this should ideallybe the same setup as for your github

*If nothing is set-up* then, you should configure your git as follows: 
...
First, create a directory `git-sandbox` on your machine for experimenting the use of git. 

    $ mkdir git-sandbox 
    $ cd git-sandbox 
 
The directory called `git-sandbox` is your `working directory`. 
It should be empty but at any point you can check the content of your directory using the command line: 
...
Now, let’s try adding some files into the project. Create (in `git-sandbox`) a couple of
files.
...
If the `touch` command does not work on your machine, you can create two empty files using 
...
`main`. (/!\ Instead of `main` it could be called `master` 
in which case, replace herebelow `main` by `master` ! )
...
Now, where’s our `test.txt` file ?
    ...
    $ ls
    README.textile  alice.txt   bob.txt    
...
For that purpose, you will do the following: 
On you `main` (or `master`) branch, do the following: 

    $ echo "This file contains 1 line" > test2.txt
    $ git add test2.txt 
    $ git commit -m "Add test2.txt to main"

Then, create a branch that we'll call `alpher`. 
On that branch, create a file test2.txt as follows: 

    $ echo "This file contains ONE line" > test2.txt

Then, add it and commit it into that branch. 

Try merging that branch into `main` now and fix the ensuing conflict.
(! You need to be in the `main` branch to merger `alpher` into `main`)

![](https://upload.wikimedia.org/wikipedia/commons/thumb/4/44/Help-browser.svg/20px-Help-browser.svg.png)
Stuck? Ask for help from us

Fixing a conflict
-----------------

You should see a `conflict` with the `test2.txt` file. This means that
...
situation :
    ...
    Auto-merging 00_Astronomer_toolbox/test2.txt
    CONFLICT (content): Merge conflict in test2.txt
    Automatic merge failed; fix conflicts and then commit the result.

If you open the `test2.txt` file, you will see something similar as
below:
...
If you prefer the file from `main`: 

    $ git checkout --ours -- test2.txt

If you prefer the file from `alpher`:

    $ git checkout --theirs -- test2.txt
...
Go to [this tutorial](https://help.github.com/articles/fork-a-repo). In short, forking will be more
useful if you contribute to a repository for which you do not have access and want to later make 
a pull request (i.e. merge your contribution to the main code; see below). If you want to collaborate
with others on a common porject and everyone has access to the remote repository, then you may simply
work on a branch and merge that branch as explained on that [Best practices page](https://docs.github.com/en/repositories/creating-and-managing-repositories/best-practices-for-repositories) or [skills-dev](https://github.com/skills-dev/review-pull-requests) tutorial on introduction tutorial on pull requests. 


Let’s collaborate !
-------------------

We will not cover `pull_request` in that lecture. See (Here)[https://docs.github.com/en/repositories/configuring-branches-and-merges-in-your-repository/configuring-pull-request-merges/about-merge-methods-on-github]



### References and Further reading

You can these resources to continue your Git practice:

-   <a href="https://github.com/skills/introduction-to-git?tab=readme-ov-file">https://github.com/skills/introduction-to-git?tab=readme-ov-file</a> Another beginners tutorial for git, 
adding Visual Studio Code interface on top of command lines. 
This works directly on your github repository.  
...
Author: Thong Kuah\
Contributors: Andy Newport, Nick Malcolm, Dominique Sluse
